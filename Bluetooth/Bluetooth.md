...
在应用层接触蓝牙已经有一段时间了，但上层能接触到的蓝牙深度还是很有限的。我所能接触到的只有系统层的API，对于底层知识，知其然，而不知其所以然。所以，最近抽空把蓝牙的底层也大概了解了一下，对相关知识点进行收集整理。由于底层很多东西涉及到电子信息工程的知识，而这些东西在毕业后就早早的还给老师了，所以有的内容也只能浅尝辄止了。本文旨在描绘蓝牙的基本轮廓，对蓝牙知识中我个人比较关心的点进行记录，以及总结回顾BLE使用过程中遇到的一些问题。
...
4. 版本1.2——2003
	...
	> 蓝牙所使用的频段和WLAN 802.11b/g冲突了  

5. 版本2.0——2004

	Bluetooth2.0+EDR是1.2的改良提升版，传输率约在1.8M_s~2.1M_s，可以有(双工)的工作方式。即一边作语音通讯，同时亦可以传输档案/高质素图片，2.0版本当然也支持Stereo运作。随后蓝牙2.0版本的芯片，增加了Stereo译码芯片，则连A2DP(Advanced Audio Distribution Profile)也可以不需要了。

	> EDR主要是使用移相键控（PSK）技术来调变RF载波使每个符号的位元数增加2到3倍。BR则使用GFSK。更详细的内容可以了解一下“调制”相关的知识  
...
> 这一层所涉及到的信号调制就直接略过了。对于频段和跳频部分，后面单独再讲。  
...
蓝牙设备集成到数字设备中的两种方式：（1）单微控制器；（2）双微控制器。在双微控制器这种模式下需要一个HCI 来胶合 主机 和 主机控制器。可以理解为一个适配器。

**L2CAP**

逻辑链路控制与适配协议（Logical Link Control  and Adaptation Protocol）

功能作用：高层协议的多路复用，数据分组的分段与重组，服务质量信息的传递。为高层协屏蔽底层传输协议的特性，适配上层和下层数据单元大小的不同。
...
接触系统上层蓝牙库想着接口的，对这个就很熟悉了。

## 应用框架（Profile）

不同的应用所使用的应用框架不同，框架内还可能嵌套其它基础框架。而一个框架则是由多个协议组成，并描述了如何使用和配置各层协议。例如要做一个蓝牙耳机产品，就去找一下对应产品的框架，然后遵守框架所描述的进行实现。这里拿两个核心的框架：GAP和GATT简单介绍一下。
...
举个例子：某米公司出了一个手环用来记录心率，并为自己的这个功能添加了一个Service（16位0xFFFF）。蓝牙手环按一定的时间间隔不停的广播自己。而在这广播数据中就有他所支持的服务。某米App通过手机扫到了这个广播，然后发起连接请求。在连接后，就可以遍历这个设备所有的Service和Service下的Characteristics。然后在0xFFFF这个服务个有两个Characteristics：0xEEE0和0xEEE1。一个是写通道，一个是接收信息的通道。写通道就是Central向Peripheral发送数据的通道，读通道就是Peripheral向Central发送数据的通道。

## BR/EDR和BLE的结构差异

大多数蓝牙芯片一都只支持蓝牙低功耗(BLE)，所以我们称之为单模芯片；而同时支持传统蓝牙和蓝牙低功耗的IC一般是手机、平板和PC机上的芯片组，我们称之为双模芯片。当然，单颗支持传统蓝牙和蓝牙低功耗的芯片有些厂家也有，只是不太常见。
...
广播类型不同，所对应的长度也不一样。广播的PDU类型又分为广播和扫描。广播的PUD类型有4种：非定向可连接；定向可连接；非定向不可连接；可被扫描的非定向广播。这里以**ADV_IND**为例学习一下
...
![](Bluetooth/70BB4651-9693-4439-BACD-E1F078A159C1.png)
...
另外通过上面的广播数据可以看到，广播中是带有蓝牙设备MAC地址的，但Apple CoreBluetooth出于某种原因将这个信息隐藏起来了，导致现在想要确定发出广播的设备是指定设备，需要将标识放在广播的自定义字段中。但Android就可以直接拿到蓝牙设备的MAC地址。

## BLE 链路层状态

BLE的LinkLayer 和BR/EDR的LinkManager对应。在LinkLayer中有6种状态：待机（Standby）、广播(Advertiser )、扫描(Scanner)、连接(Initiator)、主设备(Master)、从设备(Slave)

![](Bluetooth/C07594EC-D2E3-4717-92E6-F6F9C1828892.png)

![](Bluetooth/50F88E5C-53BB-4DDC-9F12-0AD8E2EFCB07.png)
	
上面只是一个示意图，整个连接的过程中还有很多的数据通信，但对于上层应用开发者已经被完全屏蔽掉了。这里就不深究了。


# Apple Core  Bluetooth

在iOS 13之前Core Bluetooth 只支持BLE，在iOS 13才加入BR_EDR的支持。但BR_EDR一直都有，只是没有暴露给开发者。所以，之前在播放音乐的时候，我所只能做到配置AVAudioSessionCategoryOptionAllowBluetooth让系统替我们处理了。目前可以说Core Bluetooth已经支持BR/EDR 和 BLE和。
